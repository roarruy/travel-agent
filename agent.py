...
def init_db():
    ...
    try:
        ...
        """)
        # Create wallet table with TEXT first (compatible), then migrate to JSONB
        cur.execute("""
        ...
        """)
        # Always attempt migration to JSONB (safe if already JSONB)
        cur.execute("""
            DO $$
            BEGIN
                ALTER TABLE wallet ALTER COLUMN dados TYPE JSONB USING dados::jsonb;
            EXCEPTION WHEN others THEN
                NULL; -- Already JSONB or conversion error, ignore
            END $$;
        """)
        cur.execute("""
